...
export const getCuaca = async (req, res) => {
    try { 
        ...
        const cacheKey = `page_${page}_limit_${limit}_search_${search}`;
        const cacheLama = cuacaCacheMap[cacheKey];

        // Jika cache untuk halaman & limit ini sudah ada dan belum lewat 15 menit, pakai cache ini!
        if (cacheLama && (Date.now() - cacheLama.fetchTime < waktuMinimalSelesai)) {
            console.log(`⚡ Mengambil data dari CACHE untuk: ${cacheKey}`);
            return response(200, cacheLama.data, 'Success (Cached)', res);
        }


        const rawData = fs.readFileSync(filePathWilayah, 'utf8');
        let daftarWilayah = JSON.parse(rawData);
        if (search) {
            daftarWilayah = daftarWilayah.filter(w => 
                w.label.toLowerCase().includes(search) || 
                w.id.includes(search)
            );
        }
        const totalData = daftarWilayah.length;
        const totalPages = Math.ceil(totalData / limit);
        const startIndex = (page - 1) * limit
        const endIndex = page * limit;
        const wilayahDiproses = daftarWilayah.slice(startIndex, endIndex)
        const dataCuaca = await Promise.all(
            wilayahDiproses.map(async (wilayah) => {
                try {
        ...
        cuacaCacheMap[cacheKey] = {
            fetchTime: Date.now(),
            data: hasilPagination
        };
...
// http://localhost:5000/api/cuaca?search=barus
